...
- 학습용으로 H2를 먼저 쓸지
- 실제 사용에 가까운 MySQL/PostgreSQL을 바로 쓸지
- schema 관리를 `schema.sql`로 시작할지, Flyway로 시작할지

권장 학습 순서:

1. H2 또는 Docker 기반 MySQL/PostgreSQL 중 하나 선택
2. 로컬 실행 방법 문서화
...
| 이미지 경로가 깨짐 | DB에는 `/content/...` 같은 public URL만 저장 |
