...
- 와이드 야간 사진은 홈의 full-bleed ambient background로 사용하고, 모바일 700px 안팎·데스크톱 760px 안팎의 집중된 단일 히어로 안에서 노출한다. 데스크톱에서는 이미지 하단을 기준으로 정렬해 초광폭 화면에서도 전경의 고양이가 잘리지 않게 한다.
...
- 프로젝트 카드는 텍스트 정보와 이미지 영역을 약 60:40으로 분리한다. 왼쪽에는 연도·번호, 제목, 설명, 태그를 하나의 읽기 흐름으로 배치하고 오른쪽에는 선명한 고정 이미지와 원형 arrow action을 둔다. hover·focus에서는 이미지 scale, 제목 accent, 화살표 상태만 짧게 전환하며 모바일에서는 이미지가 정보보다 먼저 보이도록 한 열로 쌓는다.
...
- 제품 화면이 여러 장이면 미디어 바로 아래에 스크롤바를 시각적으로 숨긴 가로 스크롤·scroll snap 갤러리를 두고, 터치·트랙패드 스와이프와 이전·다음 버튼을 함께 제공한다. 데스크톱에서는 한 화면에 약 3개, 모바일에서는 다음 항목이 일부 보이는 크기로 탐색 가능성을 알린다. 미리보기 이미지는 지연 로딩하고 고정 비율 프레임을 상단 기준으로 채우며, 선택하면 원본 비율을 보존하는 확대 모달에서 전체 화면을 확인한다. 모달은 닫기 버튼·바깥 영역·Escape, 내부 이전·다음 버튼·좌우 방향키, 100%부터 250%까지의 확대·축소와 확대 이미지 스크롤을 지원한다.
...
- 내부는 `max-w-[1700px]`로 제한하고 첫 행에는 브랜드·페이지 링크·소셜 링크, 둘째 행에는 저작권·`Back to top`을 배치한다.
- 브랜드는 별도 메타 라벨이나 장식선 없이 세리프 워드마크만 사용한다.
- `Navigation`, `Contact` 라벨은 사용하지 않고 둘째 행 위의 내부 hairline으로 정보 위계를 나눈다. 큰 타이포그래피, 배경 장식, 전체 패널 hover는 사용하지 않는다.
