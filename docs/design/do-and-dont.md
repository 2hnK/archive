...
- 관련 항목이 반복되면 하나의 글래스 패널 안에서 hairline으로 나눌 수 있는지 먼저 검토한다.
...
- 스크롤 상태는 현재 항목 하나만 blue accent로 표시하고, 로고는 식별이 필요하면 브랜드 색상을 상시 유지한다.
...
- 기존 모션 밀도와 맞지 않는 과도한 애니메이션, 흔들림, 반복 반짝임, 강한 glare, 3D 효과
- 기존 패턴으로 충분한데 새 프레임워크나 UI 라이브러리를 도입하는 방식
- 정적 CSS로 충분한데 React island를 추가하거나 React Bits 효과를 다른 페이지로 무분별하게 확장하는 방식
