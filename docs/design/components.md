...
- 넓은 화면에서는 첫 행에 브랜드·페이지 링크·소셜 링크를 배치하고, 둘째 행에 저작권과 `Back to top`을 배치하는 압축형 2단 구조를 사용한다.
- `Navigation`, `Contact` 같은 중간 라벨은 생략해 첫 행의 기준선과 위계를 단순하게 유지한다.
- 브랜드는 별도 메타 라벨 없이 세리프 워드마크만 사용해 첫 위계를 단순하게 유지한다.
- 저작권과 작은 원형 아이콘을 포함한 `Back to top`은 얇은 내부 hairline 아래의 둘째 행에 배치한다.
- 큰 타이포그래피, 배경 grid, 내부 카드 없이 링크 underline과 화살표의 2px 상승만 사용한다.
